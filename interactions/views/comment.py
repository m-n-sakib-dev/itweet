...
def CommentReaction(request, comment_id, reaction_type):
    ...
    reaction, created = CommentLike.objects.get_or_create(
        user=request.user, comment=comment, defaults={"reactiontype": reaction_type}
    )
    if not created:
...
def CommentReply(request, comment_id):
    ...
    if request.method == "POST":
        ...
        if comment_form.is_valid():
            ...
            comment.save()
            returncomment = commentInfo(comment)
            returncomment["parent_comment_id"] = comment_id
            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                return JsonResponse(
                    {
                        "success": True,
                    ...
                    return JsonResponse(
                        {
                            "success": True,
                            "tweet_id": tweet.id,
                            "tweet_comment_count": tweet.comment_count,
...
def EditComment(request, comment_id):
    ...
    if request.method == "POST" and request.user == comment.user:
        ...
        return JsonResponse(
            {"success": False, "errors": "only author of a comment can edit it."}
        )
