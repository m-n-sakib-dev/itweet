...
def HashtagTweetsPage(request,hashtag_name):
        ...
        if user.is_authenticated:
                hashtag_tweets_count=Hashtag.objects.filter(name=hashtag_name).count
                return render(request, 'common_page.html', {'tweet_count':hashtag_tweets_count,'page':"Hashtag Tweets",'hashtag_name':hashtag_name})
...
def HashtagTweets(request,hashtag_name):
        ...
        tweets=TweetModel.objects.filter(hashtags__name=hashtag_name).select_related('user__profile')[start:end]
